...
Build a single Expo SDK 55 application that runs on web and mobile, uses Expo Router for public and protected navigation, and relies on Supabase Auth plus TanStack Query for account lifecycle and authenticated data access. Because Supabase password sign-in is email-based, username sign-in will be implemented through a Supabase Edge Function behind the required frontend-safe wrapper in `src/modules.api.ts`.
...
**Testing**: `jest-expo` for unit and component tests, React Native Testing Library for screen behavior, contract tests for `src/modules.api.ts` and the Edge Function, local Supabase integration tests for auth flows  
...
- **Pre-Phase 0 - Frontend/Backend Separation**: PASS. UI and navigation live in `app/` and `src/`, while database schema and backend logic live under `supabase/`.
- **Pre-Phase 0 - Wrapper-Mediated Backend Access**: PASS. Screens and components consume `src/modules.api.ts`; they do not import Supabase clients or backend helpers directly.
- **Pre-Phase 0 - Boundary Verification**: PASS. The plan includes contract tests for the wrapper and Edge Function plus integration tests for redirects, verification, and recovery flows.
- **Post-Phase 1 Re-check**: PASS. The design artifacts preserve the required boundary by using a single public wrapper, a `profiles` table for usernames, and one Supabase Edge Function for username-password exchange.
...
**Structure Decision**: Use one Expo application for all UI and routing, plus one local `supabase/` directory for schema and Edge Function assets. This keeps the scaffold intentionally small while still meeting the constitution requirement that backend access be isolated behind `src/modules.api.ts`.

## Implementation Strategy

1. Establish the app shell first: Expo Router layouts, TanStack Query provider, session provider, and a wrapper-centered auth boundary with failing tests in place before implementation.
...
3. Deliver the P1 slice by wiring sign-in, protected routing, preserved `next` navigation, home, profile, and sign-out behavior against the wrapper.
